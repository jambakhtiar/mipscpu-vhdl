...
ARCHITECTURE behavior OF InterruptController IS
    ...
    SIGNAL irq_rising, irq_clr                  : STD_LOGIC_VECTOR(5 DOWNTO 0);
...
begin 
    ...
    WITH address SELECT
    CS <=   "001" WHEN "1101100", -- IE
            "010" WHEN "1101101", -- IFG
            "100" WHEN "1101110", -- TYPE
            "000" WHEN OTHERS;
    ...
    IFG_reg_in <= ("00" & irq_rising) AND IE_reg;

    INTR_reset <= '1' WHEN reset_counter = "01" ELSE '0';
    INTR <= (((IFG_reg(0) OR IFG_reg(1) OR IFG_reg(2) OR IFG_reg(3) OR IFG_reg(4) OR IFG_reg(5)) AND GIE) OR INTR_reset) AND PC_ENABLE;
	INTR_out <= INTR;
